...
public class InspectWindowUI : MonoBehaviour
{
    ...
    private GameObject SpawnInspectModel(ItemInstance item, Transform parent)
    {
        ...
        {
            model = Instantiate(item.definition.worldPrefab, parent);
        ...
        else
        {
            ...
            model.transform.localScale = new Vector3(
                Mathf.Max(0.4f, 0.25f * item.definition.width),
                0.25f,
                Mathf.Max(0.4f, 0.25f * item.definition.height));
            var r = model.GetComponent<Renderer>();
            if (r != null)
            {
                var shader = Shader.Find("Universal Render Pipeline/Lit") ?? Shader.Find("Standard");
                r.material = new Material(shader);
                r.material.color = item.definition.isContainer
                    ? new Color(0.3f, 0.5f, 0.7f)
                    : new Color(0.4f, 0.7f, 0.4f);
            }
        }

        // Отключаем всё, что может влиять на сцену
    ...
    void Update()
    {
        ...
        // Drag-rotation
        if (Input.GetMouseButtonDown(0) && IsPointerOverViewport(Input.mousePosition))
        {
            isDragging   = true;
    ...
    private void UpdateStats(ItemInstance item)
    {
        ...
        if (def.maxStackSize > 1)
            lines += $"\nСтак: {item.stackCount}/{def.maxStackSize}";
