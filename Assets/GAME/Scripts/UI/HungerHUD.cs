...
/// <summary>
/// HUD-полоска голода. Опциональный модуль — если на игроке нет
/// <see cref="HungerSystem"/>, компонент сам себя выключает (полоска не показывается).
/// </summary>
[DefaultExecutionOrder(-80)]
public class HungerHUD : MonoBehaviour
{
    public static HungerHUD Instance { get; private set; }
    ...
    [Header("Colors")]
    public Color fillColor       = new(0.95f, 0.85f, 0.20f, 1f);
    ...
    void OnDisable()
    {
        if (HungerSystem.Instance != null)
            HungerSystem.Instance.OnChanged -= Refresh;
    }

    void Start()
    {
        // Если HungerSystem появился позже Awake — досубскрайбимся.
        Subscribe();
        Refresh();

        if (HungerSystem.Instance == null && rootRt != null)
    ...
    {
        if (HungerSystem.Instance == null) return;
        HungerSystem.Instance.OnChanged -= Refresh;
        HungerSystem.Instance.OnChanged += Refresh;
    }
    ...
    {
        var go = new GameObject("HungerHUDCanvas",
    ...
    {
        var root = new GameObject("HungerBar", typeof(RectTransform));
    ...
    public void Refresh()
    {
        ...
        if (fillRt != null)
            fillRt.sizeDelta = new Vector2(innerW * v.HungerNormalized, -4f);
        if (label != null)
            label.text = $"ГОЛОД  {Mathf.RoundToInt(v.Hunger)}/{Mathf.RoundToInt(v.maxHunger)}";
    }
}
