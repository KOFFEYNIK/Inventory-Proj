...
public static class InventorySpawner
{
    ...
    public static void Spawn(Mode mode, bool withHunger, bool withThirst)
    {
        ...
        if (cam != null)
        {
            ...
            // Подключаем игровую прокладку для ПКМ-меню над предметами в мире.
            // Лейблы редактируются в инспекторе; меню легко отключается флагом useContextMenu.
            EnsureCameraComponent<WorldPickupContextMenu>(cam.gameObject, "WorldPickupContextMenu", report);
