...
public static class SampleSceneBuilder
{
    ...
    public static void Build3D_RTS()
    {
        ...
        // 3) Три юнита. Каждому даём свой RTSUnitInventory → собственное хранилище.
        // Hero — с tag Player, на нём же InventoryRig (общий UI + модули vitals).
        var hero  = Spawn3DUnit("Hero",  new Vector3(-2f, 0.5f, 0f), new Color(0.2f, 0.6f, 0.9f), "Hero");
        EnsureTag(PlayerTag); hero.tag = PlayerTag;
        Spawn3DUnit("Scout",    new Vector3( 0f, 0.5f, 0f), new Color(0.3f, 0.9f, 0.4f), "Scout");
        Spawn3DUnit("Engineer", new Vector3( 2f, 0.5f, 0f), new Color(0.9f, 0.6f, 0.3f), "Engineer");
        ...
        // Стартует с выделенным героем → его UnitInventory становится активным.
        cmd.autoSelectUnit = hero.GetComponent<RTSUnit>();
        // 4.1) Контекстное меню для ПКМ (по-умолчанию включено в RTSCommander).
        new GameObject("WorldContextMenu").AddComponent<WorldContextMenu>();
        ...
        // Сам Rig содержит UI/Hotbar/модули; данные инвентаря активного юнита подменяются
        // по событию InventoryManager.OnActiveChanged при Select() в Commander.
        // По умолчанию активен инвентарь героя (RTSCommander.Start выберет его).
    ...
    private static GameObject Spawn3DUnit(string name, Vector3 pos, Color color, string inventoryLabel = null)
    {
        ...
        // Per-unit inventory. Конфиг (если есть в проекте) даём общий — но storage у
        // каждого юнита свой, и его можно переопределить через инспектор.
        var unitInv = go.AddComponent<RTSUnitInventory>();
        unitInv.inventoryName = inventoryLabel ?? name;
