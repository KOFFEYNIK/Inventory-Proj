...
public class InventoryLayout : ScriptableObject
{
    ...
    [Tooltip("Anchored offset of the window from screen center. (0,0) = centered. " +
             "Меняется перетаскиванием за заголовок в режиме EDIT; сохраняется в JSON.")]
    public Vector2 windowPos        = Vector2.zero;
    public float   titleBarHeight   = 30f;

    [Header("Window resize limits (EDIT mode)")]
    [Tooltip("Минимальный размер окна при ресайзе за угол.")]
    public Vector2 minWindowSize    = new(480f, 360f);
    [Tooltip("Максимальный размер окна при ресайзе за угол.")]
    public Vector2 maxWindowSize    = new(1800f, 1000f);
    [Tooltip("Размер квадратной «ручки» в углах окна для ресайза (только в режиме EDIT).")]
    public float   resizeGripSize   = 18f;
