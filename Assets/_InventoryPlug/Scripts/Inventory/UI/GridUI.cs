...
public class GridUI : MonoBehaviour
{
    ...
    {
        // Габарит ячейки (footprint) — с учётом текущего поворота.
        float fw = item.CurrentWidth  * CellSize + (item.CurrentWidth  - 1) * CellGap;
        float fh = item.CurrentHeight * CellSize + (item.CurrentHeight - 1) * CellGap;

        // Корень = footprint-контейнер: фон ячейки + raycast/drag target. Сам НЕ поворачивается.
        ...
        bg.color = item.definition.isContainer
            ? new Color(0.20f, 0.30f, 0.40f, 0.95f)
            : new Color(0.28f, 0.38f, 0.28f, 0.95f);

        // Спрайт/иконка — отдельный дочерний объект, который и поворачивается при isRotated.
        var preview = ItemPreviewRenderer.GetSprite(item.definition);
        if (preview != null) CreateIcon(rt, item, preview);

        // Подпись — поверх, всегда горизонтальная (поворот не наследует от корня).
        CreateLabel(go.transform, item.definition.displayName,
                    item.CurrentWidth == 1 && item.CurrentHeight == 1 ? 9 : 11);

        // Бейдж количества в правом-нижнем углу — только для стака >1. На корне,
        // поэтому не вращается вместе с иконкой.
        if (item.stackCount > 1) CreateStackBadge(go.transform, item.stackCount);
    ...
    /// <summary>
    /// Создаёт дочерний Image со спрайтом предмета. Размер берётся по НЕповёрнутому габариту,
    /// а при <see cref="ItemInstance.isRotated"/> объект крутится на -90° (по часовой) вокруг
    /// центра — итоговый bbox совпадает с footprint ячейки. Подпись/фон остаются ровными.
    /// </summary>
    private void CreateIcon(RectTransform parent, ItemInstance item, Sprite sprite)
    {
        ...
        float uw = item.definition.width  * CellSize + (item.definition.width  - 1) * CellGap;
        float uh = item.definition.height * CellSize + (item.definition.height - 1) * CellGap;
        rt.sizeDelta        = new Vector2(uw, uh);
        rt.anchoredPosition = Vector2.zero;
        rt.localEulerAngles = item.isRotated ? new Vector3(0f, 0f, -90f) : Vector3.zero;
    ...
    /// <summary>Бейдж «×N» в правом-нижнем углу футпринта. Тень для читаемости поверх иконки.</summary>
    private static void CreateStackBadge(Transform parent, int count)
    {
        var go = new GameObject("StackBadge", typeof(RectTransform), typeof(Text), typeof(Outline));
        go.transform.SetParent(parent, false);
        var rt = go.GetComponent<RectTransform>();
        rt.anchorMin = Vector2.zero;
        rt.anchorMax = Vector2.one;
        rt.offsetMin = new Vector2(2, 1);
