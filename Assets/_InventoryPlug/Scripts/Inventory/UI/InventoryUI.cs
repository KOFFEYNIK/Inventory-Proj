...
public class InventoryUI : MonoBehaviour
{
    ...
    [Tooltip("Игнорировать сохранённый JSON-лейаут (inventory_layout.json) и брать все размеры/" +
             "позиции напрямую из Layout SO. Удобно для тонкой настройки в Inspector: правки SO " +
             "сразу видны и не маскируются пользовательскими override'ами из EDIT-режима.")]
    public bool ignoreSavedLayout = false;
    ...
    [Header("Окно ящика/контейнера")]
    [Tooltip("Нормализованная точка холста, к которой привязан центр окна ящика. " +
             "(0,0) — низ-лево, (1,1) — верх-право, (0.5,0.5) — центр. " +
             "Дефолт ≈ правее-выше центра (как на референсе).")]
    public Vector2 nestedWindowAnchor = new Vector2(0.7f, 0.62f);

    [Tooltip("Пиксельное смещение окна ящика от точки anchor.")]
    public Vector2 nestedWindowOffset = Vector2.zero;

    [Tooltip("Добавлять случайный разброс к позиции (чтобы несколько окон не ложились стопкой). " +
             "Выключи для строго фиксированной позиции.")]
    public bool nestedWindowRandomize = false;

    [Tooltip("Амплитуда случайного разброса по X/Y (px), если включён nestedWindowRandomize.")]
    public Vector2 nestedWindowRandomSpread = new Vector2(40f, 40f);
    ...
    void Awake()
    {
        ...
        customization = (ignoreSavedLayout ? null : InventoryLayoutPersistence.Load())
                        ?? new InventoryLayoutCustomization();
    ...
    /// <summary>Менеджер, чьё событие OnInventoryChanged мы сейчас слушаем (может отличаться
    /// от Instance в момент свапа). Нужен, чтобы корректно отписаться при пере-привязке.</summary>
    private InventoryManager subscribedMgr;

    void Start()
    {
        // Подпишемся на свап активного инвентаря, чтобы перестраивать UI при выборе
        // другого юнита (тактические RPG / RTS / Baldur's-style персонажи).
        InventoryManager.OnActiveChanged += HandleActiveChanged;

        SubscribeToManager(InventoryManager.Instance);
    ...
    void OnDestroy()
    {
        ...
        {
            subscribedMgr.OnInventoryChanged -= Refresh;
            subscribedMgr = null;
        }
        if (HungerSystem.Instance != null)
            HungerSystem.Instance.OnChanged -= RefreshHunger;
    ...
    private void SubscribeToManager(InventoryManager mgr)
    {
        if (subscribedMgr == mgr) return;
        if (subscribedMgr != null) subscribedMgr.OnInventoryChanged -= Refresh;
        subscribedMgr = mgr;
        if (subscribedMgr != null) subscribedMgr.OnInventoryChanged += Refresh;
    }

    private void HandleActiveChanged(InventoryManager newMgr)
    {
        SubscribeToManager(newMgr);
        // Пересоздаём equipment-слоты и контейнеры — они держат ссылки на конкретный
        // EquipmentSlot / GridContainer старого менеджера. Без ребилда UI будет показывать
        // данные предыдущего юнита.
        if (newMgr != null) RebuildInventoryPanel();
    }
    ...
    private void BuildInventoryPanel()
    {
        ...
        contentRt.anchoredPosition = Layout.windowPos;

        var blocker = content.AddComponent<Button>();
        blocker.onClick.AddListener(() => { });

        // Title bar — растянут по ширине окна (anchorMax.x = 1), чтобы корректно
        // следовать за ресайзом. Перетаскивание за заголовок двигает окно, но
        // только в режиме EDIT (EditModeOnly).
        ...
        titleRt.offsetMin = new Vector2(0f, -Layout.titleBarHeight);
        titleRt.offsetMax = new Vector2(0f, 0f);
        MakeStretchLabel("ИНВЕНТАРЬ", titleBar.transform, 16, TextAnchor.MiddleCenter);

        var titleDragger = titleBar.AddComponent<UIDragger>();
        titleDragger.RootCanvas   = rootCanvas;
        titleDragger.Target       = contentRt;
        titleDragger.EditModeOnly = true;
        titleDragger.OnMoved      = pos =>
        {
            customization.SetWindowPos(pos);
        ...
        // Угловые ручки ресайза создаём ПОСЛЕДНИМИ — так они оказываются поверх всех
        // детей окна (включая кнопку EDIT в правом верхнем углу) и корректно ловят drag.
        // Видимы/активны только в режиме EDIT.
        BuildResizeGrips(content.GetComponent<RectTransform>());
    }
    ...
    public void RebuildInventoryPanel()
    {
        ...
        // Перед пересборкой применяем все текущие overrides к runtime-копии Layout.
        // Без этого Layout.windowSize/windowPos/cellSize и т.д. остаются значениями
        // из Awake, а не теми, что пользователь выставил в EDIT-режиме или через Inspector.
        customization.ApplyTo(Layout);
        ApplyCellSizesToGrid();
        BuildInventoryPanel();
    ...
    // ── Window resize grips (corners, EDIT-mode only) ───────────────────────

    private void BuildResizeGrips(RectTransform windowRt)
    {
        float g = Layout != null ? Layout.resizeGripSize : 18f;
        CreateResizeGrip(windowRt, UIResizer.Corner.BottomRight, new Vector2(1f, 0f), g);
        CreateResizeGrip(windowRt, UIResizer.Corner.BottomLeft,  new Vector2(0f, 0f), g);
        CreateResizeGrip(windowRt, UIResizer.Corner.TopRight,    new Vector2(1f, 1f), g);
        CreateResizeGrip(windowRt, UIResizer.Corner.TopLeft,     new Vector2(0f, 1f), g);
    }

    private void CreateResizeGrip(RectTransform windowRt, UIResizer.Corner corner, Vector2 anchor, float size)
    {
        var go = new GameObject($"ResizeGrip_{corner}", typeof(RectTransform), typeof(Image));
        go.transform.SetParent(windowRt, false);
        var rt = (RectTransform)go.transform;
        rt.anchorMin = rt.anchorMax = anchor;
        rt.pivot     = anchor;       // прижимаем грип к самому углу
        rt.anchoredPosition = Vector2.zero;
        rt.sizeDelta = new Vector2(size, size);
        rt.SetAsLastSibling();        // поверх остальных детей окна

        var img = go.GetComponent<Image>();
        img.color = new Color(0.30f, 0.65f, 1f, 0.85f);
        img.raycastTarget = true;

        var resizer = go.AddComponent<UIResizer>();
        resizer.RootCanvas = rootCanvas;
        resizer.Target     = windowRt;
        resizer.GripCorner = corner;
        resizer.MinSize    = Layout != null ? Layout.minWindowSize : new Vector2(480f, 360f);
        resizer.MaxSize    = Layout != null ? Layout.maxWindowSize : new Vector2(1800f, 1000f);
        resizer.OnResized  = (sizeV, posV) =>
        {
            customization.SetWindowSize(sizeV);
            customization.SetWindowPos(posV);
            InventoryLayoutPersistence.Save(customization);
        };

        // Видим только в режиме EDIT; вне его — скрыт (не ловит клики).
        go.SetActive(false);
        LayoutEditMode.Instance?.RegisterEditOnlyVisual(go);
    }
    ...
    public void OpenNestedContainer(GridContainer container, string title)
    {
        ...
        // Центр окна привязываем к настраиваемой точке холста (nestedWindowAnchor),
        // дальше — фиксированное смещение и (опционально) случайный разброс.
        winRt.anchorMin = winRt.anchorMax = nestedWindowAnchor;
        winRt.pivot     = new Vector2(0.5f, 0.5f);
        winRt.sizeDelta = new Vector2(gridW, gridH);
        Vector2 pos = nestedWindowOffset;
        if (nestedWindowRandomize)
            pos += new Vector2(
                Random.Range(-nestedWindowRandomSpread.x, nestedWindowRandomSpread.x),
                Random.Range(-nestedWindowRandomSpread.y, nestedWindowRandomSpread.y));
        winRt.anchoredPosition = pos;
    ...
    /// <summary>Лейбл, растянутый на всего родителя (следует за ресайзом title bar).</summary>
    private static void MakeStretchLabel(string text, Transform parent, int fontSize, TextAnchor anchor)
    {
        var go = new GameObject("Lbl_" + text, typeof(RectTransform), typeof(Text));
        go.transform.SetParent(parent, false);
        var rt = go.GetComponent<RectTransform>();
